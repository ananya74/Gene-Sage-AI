...
#deep learning functions libcudnn8 libcudnn8-dev(compile code that uses cudnn)
...
def run_brca1_analysis():
    ...
    # Fix: register TE precision override so torch.load works
    
    import torch
    # ✅ Allowlist transformer_engine custom class
    import transformer_engine.common.recipe as te_recipe
    torch.serialization.add_safe_globals([te_recipe._OverrideLinearPrecision])
...
def get_genome_sequence(position,genome:str,chromosome:str,window_size=8192):
    ...
    if len(sequence) != expected_length:
        print(f"Warning : received sequence length ({len(sequence)}) does not match expected length {expected_length}")
...
def analyze_variant(relative_pos_in_window, reference, alternative, window_seq, model ):
    ...
    #Threshold:Confidence Params: {'threshold': np.float32(-0.0009178519), 'lof_std': np.float32(0.0015140239), 'func_std': np.float32(0.0009016589)}
...
class Evo2Model:
    ...
    def analyze_single_variant(self,variant_position:int, alternative:str, genome:str, chromosome:str ):
        ...
        if relative_pos < 0 or relative_pos >= len(window_seq):
            raise ValueError(f"Variant position: {variant_position} is out of the fetched sequence window (start={seq_start+1}, end={seq_start+len(window_seq)})")
        

        refernce=window_seq[relative_pos]
        print("Reference is:"+ refernce)
...
def main():
    ...
    print("Classification" + str(result["classification_confidence"]))


    # in power shell use Invoke-WebRequest `     
        #>>   -Uri "https://ananya74--variant-analysis-evo2-evo2model-analyze-single-bcda61.modal.run/?variant_position=43119628&alternative=G&genome=hg38&chromosome=chr17" `
        #>>   -Method POST